...
Für jeden dieser Fälle können nun die Positionsänderungen $\Delta\gamma$, $\Delta x_0$, $\Delta y_0$ im roboterfesten KS vorberechnet werden.
\begin{flalign}
    arc_{Rn} & = \frac{2\pi\cdot r_R}{n_{Enc}}  \\
	\Delta\gamma & = \frac{arc_{Rn}}{l_a}  \\
	\Delta x_0 & = \frac{1}{2}l_a\sin{(\Delta\gamma)}  \\
	\Delta y_0 & = \frac{1}{2}l_a ( \cos{(\Delta\gamma)} - 1) 
\end{flalign}
